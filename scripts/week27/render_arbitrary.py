...
def render_sets(dataset : ModelParams, hyperparam, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, skip_video: bool):
    with torch.no_grad():
        ...
        if not skip_video:
            video_cameras_ds: FourDGSdataset = scene.getVideoCameras()
            cameras: List[Camera] = video_cameras_ds.dataset
            new_cameras: List[Camera] = [deepcopy(cameras[85]) for _ in cameras]
            cams = []
            for i in range(len(new_cameras)):
                cams.append(new_cameras[i]._replace(time=cameras[i].time))
            new_video_cameras_ds: FourDGSdataset = FourDGSdataset(
                cams,
                args=video_cameras_ds.args,
                dataset_type=video_cameras_ds.dataset_type
            )
            render_set(dataset.model_path,"video",scene.loaded_iter,new_video_cameras_ds,gaussians,pipeline,background,cam_type)
